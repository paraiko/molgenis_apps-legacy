...
public class LocationManagementPlugin extends PluginModel<Entity> {
	...
	SimpleDateFormat newDateOnlyFormat = new SimpleDateFormat("yyyy-MM-dd",
			Locale.US);
	...
	public void handleRequest(Database db, Tuple request) {
		...
		try {
			...
			if (animalsInLocMatrixViewer != null
					&& action.startsWith(animalsInLocMatrixViewer.getName())) {
			...
			if (animalsNotInLocMatrixViewer != null
					&& action.startsWith(animalsNotInLocMatrixViewer.getName())) {
			...
			if (action.equals("AddAnimals")) {
				...
				animalsNotInLocMatrixViewer = new MatrixViewer(this,
						ANIMALSNOTINLOCMATRIX, new SliceablePhenoMatrix(
								Individual.class, Measurement.class), true, 2,
						false, true, null, new MatrixQueryRule(
			...
			if (action.equals("ApplyAddAnimals")) {
				...
				List<ObservationElement> rows = (List<ObservationElement>) animalsNotInLocMatrixViewer
						.getSelection(db);
				int rowCnt = 0;
				for (ObservationElement row : rows) {
					if (request.getBool(ANIMALSNOTINLOCMATRIX + "_selected_"
							+ rowCnt) != null) {
						assignAnimalToLocation(db, invName, row.getName(),
								locName, startDate);
					}
					rowCnt++;
				}
			...
			if (action.equals("Move")) {
				...
				Date startDate = newDateOnlyFormat.parse(startDateString);
				List<ObservationElement> rows = (List<ObservationElement>) animalsInLocMatrixViewer
						.getSelection(db);
				int rowCnt = 0;
				for (ObservationElement row : rows) {
					if (request.getBool(ANIMALSINLOCMATRIX + "_selected_"
							+ rowCnt) != null) {
						assignAnimalToLocation(db, invName, row.getName(),
								newLocationName, startDate);
					}
					rowCnt++;
				}
				prepareInLocMatrix(db, newLocationName);
				action = "Manage";
				this.setSuccess("Animals successfully moved to "
						+ newLocationName + ". Now switching to that location.");
				return;
			}
	...
	private void prepareInLocMatrix(Database db, String locationName)
			throws Exception {
		...
		filterRules.add(new MatrixQueryRule(
				MatrixQueryRule.Type.colValueProperty, ct
						.getMeasurementId("Location"),
				ObservedValue.RELATION_NAME, Operator.EQUALS, locationName));
		filterRules.add(new MatrixQueryRule(
				MatrixQueryRule.Type.colValueProperty, ct
						.getMeasurementId("Location"), ObservedValue.ENDTIME,
		...
		animalsInLocMatrixViewer = new MatrixViewer(this, ANIMALSINLOCMATRIX,
				new SliceablePhenoMatrix<Individual, Measurement>(
						Individual.class, Measurement.class), true, 2, false,
				true, filterRules, new MatrixQueryRule(
	...
	private void assignAnimalToLocation(Database db, String investigationName,
			String animalName, String locationName, Date startDate)
		...
		db.add(ct.createObservedValueWithProtocolApplication(investigationName,
				startDate, null, "SetLocation", "Location", animalName, null,
				locationName));
	}
	...
	public void reload(Database db) {
		...
		try {
			...
			List<String> locationNameList = ct.getAllObservationTargetNames(
					"Location", false, investigationNames);

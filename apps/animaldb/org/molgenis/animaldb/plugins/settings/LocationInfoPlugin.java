...
public class LocationInfoPlugin extends PluginModel<Entity> {
	...
	private String getSuperLoc(Database db, String locName)
	...
	public void handleRequest(Database db, Tuple request) {
		...
		try {
			...
			if (action.equals("importLocations")) {
				...
				ImportAteLocations importer = new ImportAteLocations(db,
						this.getLogin());
			...
			if (action.equals("addLocation")) {
				...
				if (slocName != null && !slocName.equals("")) {
					db.add(ct.createObservedValueWithProtocolApplication(
							invName, new Date(), null, "SetSublocationOf",
	...
	public void reload(Database db) {
		...
		try {
			...
			for (ObservationTarget loc : locationList) {
				superLocMap.put(loc.getId(),
						this.getSuperLoc(db, loc.getName()));
			}

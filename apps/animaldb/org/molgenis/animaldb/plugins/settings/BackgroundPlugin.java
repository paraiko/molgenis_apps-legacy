...
public class BackgroundPlugin extends PluginModel<Entity> {
	...
	public void handleRequest(Database db, Tuple request) {
		...
		try {
			...
			if (action.equals("addBackground")) {
				...
				// ct.getOwnUserInvestigationName(this.getLogin().getUserName());
				String investigationName = "System"; // TODO remove old
														// investigation code
														// above.
				ct.makePanel(investigationName, bkgName, this.getLogin()
						.getUserName());
				db.add(ct.createObservedValueWithProtocolApplication(
						investigationName, new Date(), null, "SetTypeOfGroup",
						"TypeOfGroup", bkgName, "Background", null));
				db.add(ct.createObservedValueWithProtocolApplication(
						investigationName, new Date(), null, "SetSpecies",
	...
	public void reload(Database db) {
		...
		try {
			...
			backgroundList = ct.getAllMarkedPanels("Background",
					investigationNames);
			...
			for (ObservationTarget background : backgroundList) {
				String speciesName = ct.getMostRecentValueAsXrefName(
						background.getName(), "Species");
				if (speciesName != null) {
